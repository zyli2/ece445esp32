...
#define ADV_INT_MS(ms) ((uint16_t)(((ms) * 1000) / 625))
...
static void start_advertising(void) {
    ...
    // Optionally, scan response (empty or shortened name)
    rsp_fields.name = (uint8_t *)name;
    rsp_fields.name_len = strlen(name);
    rsp_fields.name_is_complete = 0;  // Can be shortened
    ...
    adv_params.itvl_min = ADV_INT_MS(500);  // 500ms
    adv_params.itvl_max = ADV_INT_MS(510);  // 510ms
